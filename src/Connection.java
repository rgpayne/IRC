...
public class Connection implements Runnable {
    ...
    public static int findTab(final String title, final Connection conn) {
        int totalTabs = GUI.getTabbedPane().getTabCount();
     ...
     */
    public void parseFromServer(final String line) throws IOException, BadLocationException {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (command.equals("ERROR")) {
                            ChannelPanel channel = (ChannelPanel) GUI.getTabbedPane().getSelectedComponent();
                            if (channel == null) {
                                ChannelPanel cp = null;
                                cp = new ChannelPanel(Connection.this.title, "", Connection.this);
                                String[] msg = {null, parser.getTrailing()};
                                cp.insertString(msg, GUI.connectStyle, false);

                                return;
                            }
                            if (!Connection.this.isConnected && line.startsWith("ERROR :Closing Link"))
                                return; //standard disconnection message when we /quit
                            return;
                        }
                        ...
                        if (command.equals("MODE")) {
                            ...
                            } else //setting a user's mode
                            {
                                ...
                                int indexOfChannel = findTab(chan, Connection.this);
                        ...
                        if (command.equals("NOTICE")) {
                            ...
                            int indexOfChannel = findTab(Connection.this.title, Connection.this);
                            if (indexOfChannel == -1) {
                            ...
                            ChannelPanel channel = (ChannelPanel) GUI.getTabbedPane().getSelectedComponent();
                            if (channel.connection != Connection.this) {
                                indexOfChannel = findTab(title, Connection.this);
                        ...
                        if (command.equals("PART")) {
                            ...
                            if (currentNick.equals(parser.getNick())) { //if i'm leaving
                                GUI.getTabbedPane().remove(indexOfChannel);
                                return;
                            } else {
                                ChannelPanel channel = (ChannelPanel) GUI.getTabbedPane().getComponentAt(indexOfChannel);
                                if (!GUI.hideJoinPartQuitNotifications) {
                        ...
                        if (command.equals("PRIVMSG") || command.equals("MSG")) {
                            ...
                            if (parser.getTrailing().startsWith(CTCP_DELIM)) {
                                ...
                                if (parser.getTrailing().substring(1).startsWith("CLIENTINFO")) //CTCP clientinfo
                                {
                                    ...
                                    channel.insertString(msg, GUI.connectStyle, false);
                                    Connection.this.send("NOTICE " + parser.getNick() + " :\001CLIENTINFO CTCP commands: ACTION FINGER PING SOURCE TIME USERINFO VERSION\001");
                                    return;
                                }
                                if (parser.getTrailing().substring(1).startsWith("FINGER")) //CTCP Finger
                                {
                                    ...
                                    channel.insertString(msg, GUI.connectStyle, false);
                                    Connection.this.send("NOTICE " + parser.getNick() + " :\001FINGER " + ChannelPanel.CTCPFingerMessage + "\001");
                                    return;
                                }
                                if (parser.getTrailing().substring(1).startsWith("PING")) //CTCP Ping
                                {
                                    ...
                                    channel.insertString(msg, GUI.connectStyle, false);
                                    Connection.this.send("NOTICE " + parser.getNick() + " :" + parser.getTrailing());
                                    return;
                                }
                                if (parser.getTrailing().substring(1).startsWith("SOURCE")) //CTCP source
                                {
                                    ...
                                    channel.insertString(msg, GUI.connectStyle, false);
                                    Connection.this.send("NOTICE " + parser.getNick() + " :\001SOURCE Unavailable\001");
                                    return;
                                }
                                if (parser.getTrailing().substring(1).startsWith("TIME")) //CTCP Time
                                {
                                    ...
                                    Connection.this.send("NOTICE " + parser.getNick() + " :\001TIME " + time + "\001");
                                    return;
                                }
                                if (parser.getTrailing().substring(1).startsWith("USERINFO")) //ctcp userinfo
                                {
                                    ...
                                    channel.insertString(msg, GUI.connectStyle, false);
                                    Connection.this.send("NOTICE " + parser.getNick() + " :\001USERINFO " + ChannelPanel.CTCPUserInfo + "\001");
                                    return;
                                }
                                if (parser.getTrailing().substring(1).startsWith("VERSION")) //CTCP Version
                                {
                                    ...
                                    channel.insertString(msg, GUI.connectStyle, false);
                                    Connection.this.send("NOTICE " + parser.getNick() + " :\001VERSION AlphaClient:v0.1:LM17\001");  //placeholder
                                    return;
                                } else {
                                    ChannelPanel channel = (ChannelPanel) GUI.getTabbedPane().getSelectedComponent();
                        ...
                        if (command.equals("QUIT")) {
                            ...
                            for (int i = 0; i < GUI.getTabbedPane().getTabCount(); i++) {
                                ChannelPanel channel = (ChannelPanel) GUI.getTabbedPane().getComponentAt(i);
                                if (channel.connection == Connection.this) {
                                    int index = findTab(channel.title, Connection.this);
                                    if (index != -1) {
                        ...
                        if (command.equals("001") || command.equals("002") || command.equals("003") || command.equals("004") || command.equals("005")) {
                            ...
                            indexOfChannel = findTab(Connection.this.title, Connection.this);
                            ChannelPanel channel = (ChannelPanel) GUI.getTabbedPane().getComponentAt(indexOfChannel);
                            ...
                            if (text.equals(""))
                                text = parser.getParams().trim().substring(currentNick.length()).trim();
                            String[] msg = {null, text};
                            channel.insertString(msg, GUI.connectStyle, ctcp);

                            if (command.equals("001") && Connection.this.autoconnect) {
                                try {
                                    Thread.sleep(1000);
                        ...
                        {
                            int index = findTab(Connection.this.title, Connection.this);
                            ChannelPanel channel = (ChannelPanel) GUI.getTabbedPane().getComponentAt(index);
                            String[] msg = {null, parser.getParams().trim() + "."};
                            channel.insertString(msg, GUI.connectStyle, false);
                            return;
                        }
                        ...
                        if (command.equals("221")) //requesting to see own modes (/modes rieux)
                        {
                            ...
                            ChannelPanel channel;
                            if (indexOfChannel == -1)
                                channel = (ChannelPanel) GUI.getTabbedPane().getSelectedComponent();
                            else channel = (ChannelPanel) GUI.getTabbedPane().getComponentAt(indexOfChannel);
                        ...
                        if (command.equals("331")) //no topic
                        {
                            ...
                            String[] msg = {null, parser.getTrailing()};
                            int channelIndex = findTab(channelName, Connection.this);
                            ChannelPanel channel = (ChannelPanel) GUI.getTabbedPane().getComponentAt(channelIndex);
                            channel.topic = "";
                            channel.insertString(msg, GUI.serverStyle, false);
                            return;
                        }
    ...
    public void disconnect() {
        try {
            Connection.this.send("QUIT :"+GUI.quitMessage);
